...
class WindowsRegistry 
{
	...
	public function ReadValue($keyPath, $valueName, $asString = TRUE)
	{
		...
		switch ($valueType)
		{
			...
			case self::REG_DWORD:
				...
				break;
			case self::REG_QWORD:
				$result = $this->RegObject->GetQWORDValue($hKey, $subKey, $valueName, $valueContents);
	...
	public function ReadType($keyPath, $valueName, $asString = TRUE)
	{
		$hKey = 0;
		$subKey = "";
		if (!$this->SplitKeyPath($keyPath, $hKey, $subKey))
		...
		switch ($valueType)
		{
			...
			case self::REG_EXPAND_SZ:
				$resultValue = "REG_EXPAND_SZ";
				break;

			case self::REG_BINARY:
				$resultValue = "REG_BINARY";
				break;

			case self::REG_DWORD:
				$resultValue = "REG_DWORD";
				break;

			case self::REG_LINK:
				$resultValue = "REG_LINK";
				break;

			case self::REG_MULTI_SZ:
				$resultValue = "REG_MULTI_SZ";
				break;

			case self::REG_RESOURCE_LIST:
				$resultValue = "REG_RESOURCE_LIST";
				break;

			case self::REG_FULL_RESOURCE_DESCRIPTOR:
				$resultValue = "REG_FULL_RESOURCE_DESCRIPTOR";
				break;

			case self::REG_RESOURCE_REQUIREMENTS_LIST:
				$resultValue = "REG_RESOURCE_REQUIREMENTS_LIST";
				break;

			case self::REG_QWORD:
				$resultValue = "REG_QWORD";
				break;
