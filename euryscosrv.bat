...
if %relpath:~1,-1% neq agent if %phpexe:~9,-1% equ core echo 		ProxyPass http://127.0.0.1:%phpport:~1,-1%/ Timeout=180 KeepAlive=On>>"%cd%\apache_%osarc%\conf\httpd_%phpexe:~1,-1%_virtual.conf"
if %relpath:~1,-1% neq agent if %phpexe:~9,-1% equ executor echo 		ProxyPass http://127.0.0.1:%phpport:~1,-1%/ Timeout=2000 KeepAlive=On>>"%cd%\apache_%osarc%\conf\httpd_%phpexe:~1,-1%_virtual.conf"
if %relpath:~1,-1% neq agent if %phpexe:~9,-1% equ server echo 		ProxyPass http://127.0.0.1:%phpport:~1,-1%/ Timeout=35 KeepAlive=On>>"%cd%\apache_%osarc%\conf\httpd_%phpexe:~1,-1%_virtual.conf"
