...
<?php if (!isset($_GET['csv_nodes'])) { $_SESSION['nodes'] = $_SERVER['REQUEST_URI']; } ?>
...
<script type="text/javascript">
	function nodeinfo(ComputerName,Domain,ComputerIP,LastCom,Manufacturer,Model,OSName,CPUModel,ManufacturerFull,ModelFull,OSNameFull,CPUModelFull,DomainFull,ComDom,ComDomFull,Inventory,Programs,CorePort,ExecutorPort,Find,FindType,FilterPrograms,AgentVersion,ConnectionStatus){
		...
		$.Dialog({
			...
			'content'     : '<table width="100%" border="0" cellspacing="0" cellpadding="0" class="striped"><tr class="rowselectsrv"><td colspan="2" style="font-size:12px;" Title="' + ComDomFull + '">' + ToRemote + ComDom + '</td></tr><tr class="rowselectsrv"><td style="font-size:12px;">Domain:&nbsp;</td><td style="font-size:12px;"><a href="/nodes.php?find=domain.' + DomainFull + '&findtype=status" style="font-size:12px;" title="Filter by Domain: ' + DomainFull + '">' + Domain + '</a></td></tr><tr class="rowselectsrv"><td style="font-size:12px;">IP Address:&nbsp;</td><td style="font-size:12px;">' + ComputerIP + '</td></tr><tr class="rowselectsrv"><td style="font-size:12px;"><a href="/xml.php?export=status&source=' + ComputerName + '" style="font-size:12px;" title="Export Source XML"><div class="icon-file-xml"></div></a>&nbsp;Last&nbsp;Update:&nbsp;</td><td style="font-size:12px;">' + LastCom + '</td></tr><tr class="rowselectsrv"><td style="font-size:12px;">Manufacturer:&nbsp;</td><td style="font-size:12px;"><a href="/nodes.php?find=' + ManufacturerFull + '&findtype=status" style="font-size:12px;" title="Filter by Manufacturer: ' + ManufacturerFull + '"><div class="icon-tag"></div>&nbsp;' + Manufacturer + '</a></td></tr><tr class="rowselectsrv"><td style="font-size:12px;">Model:&nbsp;</td><td style="font-size:12px;"><a href="/nodes.php?find=' + ModelFull + '&findtype=status" style="font-size:12px;" title="Filter by Model: ' + ModelFull + '"><div class="icon-monitor"></div>&nbsp;' + Model + '</a></td></tr><tr class="rowselectsrv"><td colspan="2" style="font-size:12px;" title="' + CPUModelFull + '"><a href="/nodes.php?find=' + CPUModelFull + '&findtype=status" style="font-size:12px;" title="Filter by CPU Model: ' + CPUModelFull + '"><div class="icon-checkbox-partial"></div>&nbsp;' + CPUModel + '</a></td></tr><tr class="rowselectsrv"><td colspan="2" style="font-size:12px;" title="' + OSNameFull + '"><a href="/nodes.php?find=' + OSNameFull + '&findtype=status" style="font-size:12px;" title="Filter by OS Name: ' + OSNameFull + '"><div class="icon-windows"></div>&nbsp;' + OSName + '</a></td></tr><tr class="rowselectsrv"><td style="font-size:12px;"><form id="agentrefreshform" name="agentrefreshform" method="post">' + AgentRestart + 'eurysco Agent:&nbsp;</form></td><td style="font-size:12px;">Version ' + AgentVersion + '</td></tr></table>',
			'draggable'   : true,
			'overlay'     : true,
			'closeButton' : false,
			'buttonsAlign': 'center',
...
<script type="text/javascript">
	function resetallagent(){
		$.Dialog({
			...
			'content'     : '<span style="font-size:12px;">Restart Confirmation...</span><form id="agentrefreshform" name="agentrefreshform" method="post"><input type="hidden" id="agentrefresh" name="agentrefresh" value="" /></form>',
			'draggable'   : true,
			'overlay'     : true,
			'closeButton' : false,
			'buttonsAlign': 'center',
		...
		<div class="page-region-content">
			<div class="grid">
				<div class="row">
					...
					if (isset($_POST['agentrefresh']) && ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usersett']['nodesstatus'] > 1) && (strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME'])) > 0) {
						if ($_POST['agentrefresh'] != '') {
							$mcrykeycmd = pack('H*', hash('sha256', md5(strtolower($_POST['agentrefresh']))));
							$cid = '';
							$exectimeout = 10000;
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="striped">
						...
						<?php if ($_SESSION['cmdnodes'] != '') { ?><tr><td width="20%"><div style="font-size:12px; white-space:nowrap; table-layout:fixed; overflow:hidden;">Command Result:</div></td><td width="80%" style="font-size:12px;"><div id="cmdresult" style="font-size:12px;"></div></td></tr><?php } ?>
                    </table>
					...
					<form id="meteringform" name="meteringform" method="get">
						<input type="hidden" id="orderby" name="orderby" value="<?php echo $orderby; ?>" />
						<input type="hidden" id="find" name="find" value="<?php echo $find; ?>" />
					...
					<ul class="accordion" data-role="accordion">
						<li<?php echo $cmdacco; ?>>
							...
							<form id="clicommand" name="clicommand" onsubmit="if(!confirmcommand()){return false;}" method="post">
								<div class="input-control input span8">
					...
					<?php
					$meteringpath = str_replace('\\core', '\\server\\metering', $_SERVER['DOCUMENT_ROOT']);
					$metering = scandir($meteringpath);
					
					if (isset($_GET['meteringactive'])) { $meteringacco = ' class="active"'; } else { $meteringacco = ''; }
					
					$meteringname = $meteringpath . '\\' . (count($metering) - 1) . '_' . md5($find) . '.' . $findtype . '_metering';
					if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usersett']['nodesstatus'] > 1) {
						if (isset($_GET['metering'])) {
							if ($_GET['metering'] == 'start' && !file_exists($meteringname)) {
					...
					function update() {
						$.ajax({
							type: "GET",
							url: 'nodesjq.php?orderby=<?php echo $orderby; ?>&find=<?php echo urlencode($find); ?>&findtype=<?php echo urlencode($findtype); ?>&page=<?php echo $pgkey; ?>&phptimeout=<?php echo $phptimeout; ?>&confirmdeploy=<?php echo $confirmdeploy; ?><?php if ($sessionfind != '') { echo '&sessionfind=' . $sessionfind; } ?>&results=<?php echo $results; ?>',
							data: '',
							dataType: 'json',
							cache: false,
							...
							if (data.filterprogress == 0) { $('#sessionfindres').html('<i><?php echo $find; ?></i><?php if (!strpos($findtype, '_metering')) { ?>&nbsp;&nbsp;<a href="?orderby=<?php echo $orderby; ?>&find=<?php echo urlencode($find); ?>&findtype=<?php echo urlencode($findtype); ?>&page=<?php echo $pgkey + 1; ?>&results=<?php echo $resultssw; ?>" style="font-size:14px;" title="<?php echo $resultsti; ?>"><div class="<?php echo $resultsic; ?>"></div></a><?php } ?>'); } else { $('#sessionfindres').html('<i><?php echo $find; ?></i>'); }
							}
						});
					}
					</script>

					<form id="nodeinfo" name="nodeinfo" method="get">
