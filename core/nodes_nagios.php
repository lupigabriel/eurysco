...
<?php if (!isset($_GET['csv_nodes_nagios'])) { $_SESSION['nodes_nagios'] = '<a href="' . $_SERVER['REQUEST_URI'] . '" title="Nagios Status"><img src="/img/nagios_icon.png" width="20" height="16" style="vertical-align: middle;" /> ' . $node . '</a>'; } ?>
			...
			<div class="grid">
				<div class="row">
					...
					function update() {
						$.ajax({
							type: "GET",
							url: 'nodes_nagiosjq.php?orderby=<?php echo $orderby; ?>&node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>&filter=<?php echo urlencode($filter); ?>&page=<?php echo $pgkey; ?>&phptimeout=<?php echo $phptimeout; ?>',
							data: '',
							dataType: 'json',
							cache: false,
							...
							$('#lastupdate').html('<?php if ($_SESSION['usersett']['nodesnagiosstatusf'] == '') { ?><a href="/xml.php?export=nagios&source=<?php echo $node; ?>" style="font-size:12px;" title="Export Source XML"><div class="icon-file-xml"></div></a><?php } ?>Last&nbsp;Update:&nbsp;' + data.lastupdate);
