...
<?php if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usertype'] == 'Users' || $_SESSION['usersett']['nodesservicecontrol'] > 0) {  } else { header('location: /'); exit; } ?>
...
if (isset($_GET['csv_nodes_services'])) {
	$_SESSION['csv_nodes_services'] = 'csv_nodes_services';
} else {
...
<script type="text/javascript">
	function serviceexec(ProcessId,DisplayName,Name,State,StartMode,ExitCode,StartName,LimitName,ShortName,UrlName){
		$.ajax({
			...
			if (data.Name == '') { $('#Name').html('-'); } else { $('#Name').html('<?php if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usertype'] == 'Users' || $_SESSION['usersett']['nodesprocesscontrol'] > 0) { ?><a href="/nodes_processes.php?filter=' + data.NameURL + '&node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>" style="font-size:12px;" title="Filter Processes by Process Name"><div class="icon-bars"></div><?php } ?>' + data.NamePath + '<?php if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usertype'] == 'Users' || $_SESSION['usersett']['nodesprocesscontrol'] > 0) { ?></a><?php } ?>'); }
		...
		$.Dialog({
			...
			'buttons'     : {
				<?php if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usersett']['nodesservicecontrol'] > 1) { ?>
				'Start'     : {
				'action': function(){
...
<div class="page secondary">
	<div class="page-header">
		<div class="page-header-content">
            ...
            <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>" class="eurysco-services-button big page-back"></a>
		</div>
	</div>
</div>
				...
				<div class="row">
					...
					if ($serviceidexec != '' && $servicetypeexec != '' && (strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME'])) > 0) {
						...
						if ($servicetypeexec == 1) {
							...
							$xml = $xml . '	<auditko>' . base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode('     ' . $_SESSION['username'] . '     ' . $node . '     service control     service "' . $servicenameexec . '" not started (command sent from server "' . $envcomputername . '")')))))) . '</auditko>' . "\n";
							$xml = $xml . '	<auditnl>' . base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode('     ' . $_SESSION['username'] . '     ' . $node . '     service control     service "' . $servicenameexec . '" already been started (command sent from server "' . $envcomputername . '")')))))) . '</auditnl>' . "\n";
							$xml = $xml . '	<cid>' . $cid . '</cid>' . "\n";
							$xml = $xml . '	<timeout>' . $exectimeout . '</timeout>' . "\n";
							$xml = $xml . '	<command>' . base64_encode($iv . mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $mcrykeycmd, $serviceexecoutput, MCRYPT_MODE_CBC, $iv)) . '</command>' . "\n";
							$xml = $xml . '</exec>';
							$fp = fopen($nodepath . date('YmdHis') . md5($serviceidexec . 'process') . '.exec', 'w');
							fwrite($fp, $xml);
							fclose($fp);
						...
						if ($servicetypeexec == 2) {
							...
							$xml = $xml . '	<auditko>' . base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode('     ' . $_SESSION['username'] . '     ' . $node . '     service control     service "' . $servicenameexec . '" not stopped (command sent from server "' . $envcomputername . '")')))))) . '</auditko>' . "\n";
							$xml = $xml . '	<auditnl>' . base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode('     ' . $_SESSION['username'] . '     ' . $node . '     service control     service "' . $servicenameexec . '" already been stopped (command sent from server "' . $envcomputername . '")')))))) . '</auditnl>' . "\n";
							$xml = $xml . '	<cid>' . $cid . '</cid>' . "\n";
							$xml = $xml . '	<timeout>' . $exectimeout . '</timeout>' . "\n";
							$xml = $xml . '	<command>' . base64_encode($iv . mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $mcrykeycmd, $serviceexecoutput, MCRYPT_MODE_CBC, $iv)) . '</command>' . "\n";
							$xml = $xml . '</exec>';
					...
					if ($startupname != '' && $startuptype != '' && (strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME'])) > 0) {
						...
						$xml = '<exec>' . "\n";
						$xml = $xml . '	<auditok>' . base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode('     ' . $_SESSION['username'] . '     ' . $envcomputername . '     service control     service "' . $servicenameexec . '" changed to "' . $startuptype . '" (command sent from server "' . $envcomputername . '")')))))) . '</auditok>' . "\n";
						$xml = $xml . '	<auditko>' . base64_encode(base64_encode(base64_encode(base64_encode(base64_encode(base64_encode('     ' . $_SESSION['username'] . '     ' . $envcomputername . '     service control     service "' . $servicenameexec . '" not changed to "' . $startuptype . '" (command sent from server "' . $envcomputername . '")')))))) . '</auditko>' . "\n";
