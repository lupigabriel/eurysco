		...
		<div class="page-region-content">
			<div class="grid">
				<div class="row">
					...
					<?php } ?>
					<?php if ($_SESSION['username'] != 'Administrator' && $newgroup['name'] == 'eurysco Services') { ?>
					<div style="font-size:12px;">* only administrator can manage this group</div>
					<?php } ?>
					<ul class="accordion" data-role="accordion">
							...
							foreach ($grouplist as $group) {
								if (pathinfo($group)['extension'] == 'xml' && filesize($euryscoinstallpath . '\\groups\\' . $group) > 0) {
									...
									if (hash('sha512', $userxml->settings->username . $group) == $userxml->settings->usertype) {
										...
										$usertype = $group;
										$usertypeintegrity = 1;
										if ($group == 'eurysco Services' && $_SESSION['username'] != 'Administrator') { $userchkmgnt = ''; } else { $userchkmgnt = '<a class="place-right" href="javascript:changeuser(\'' . $user . '\',\'' . $userxml->settings->username . '\',\'' . $group . '\',\'' . $userauth . '\');" title="Change: ' . $userxml->settings->username . '"><div class="icon-key"></div></a>'; }
										if ($userxml->settings->username == $_SESSION['username'] || (hash('sha512', $userxml->settings->username . 'Distributed') == $userxml->settings->userauth && $serverstatus == 'cfg' && $agentstatus != 'cfg')) { $userrow = '<div name="' . $userxml->settings->username . '"></div>' . $userauthicon . $userxml->settings->username . $userstatus . '</blockquote><br />'; } else { $userrow = '<div name="' . $userxml->settings->username . '"></div>' . $userauthicon . $userxml->settings->username . $userstatus . $userchkmgnt . '</blockquote><br />'; }
									}
								}
							}
