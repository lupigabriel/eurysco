...
<?php if (!isset($_GET['csv_nodes_programs'])) { $_SESSION['nodes_programs'] = '<a href="' . $_SERVER['REQUEST_URI'] . '" title="Installed Programs"><div class="icon-checkmark"></div>' . $node . '</a>'; } ?>
		...
		<div class="page-region-content">
			<div class="grid">
				<div class="row">
					...
					if ($uninstallidprogram != '' && $uninstallprogramconf != '' && (strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME'])) > 0) {
						$cid = md5(date('r') . $_SESSION['username'] . $node);
						if ($uninstallprogramconf == 1 && ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usersett']['nodesinstalledprograms'] > 1)) {
							...
							fclose($fp);
							$message = '<blockquote style="font-size:12px; background-color:#0072C6; color:#FFFFFF; border-left-color:#324886;">uninstall <strong>' . $uninstallnameprogram . '</strong>' . $uninstallversprogramh . ' command sent to node <strong>' . $node . '</strong></blockquote><br />';
					...
					function update() {
						$.ajax({
							type: "GET",
							url: 'nodes_programsjq.php?orderby=<?php echo $orderby; ?>&node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>&filter=<?php echo urlencode($filter); ?>&page=<?php echo $pgkey; ?>&phptimeout=120&cid=<?php echo $cid; ?>&message=<?php echo urlencode($message); ?>',
							data: '',
							dataType: 'json',
							cache: false,
							timeout: 120000,
							...
							$('#lastupdate').html('<?php if ($_SESSION['usersett']['nodesinstalledprogramsf'] == '') { ?><a href="/xml.php?export=programs&source=<?php echo $node; ?>" style="font-size:12px;" title="Export Source XML"><div class="icon-file-xml"></div></a><?php } ?>Last&nbsp;Update:&nbsp;' + data.lastupdate);
