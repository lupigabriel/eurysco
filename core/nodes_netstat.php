...
<?php if (!isset($_GET['csv_nodes_netstat'])) { $_SESSION['nodes_netstat'] = '<a href="' . $_SERVER['REQUEST_URI'] . '" title="Network Stats"><div class="icon-tab"></div>' . $node . '</a>'; } ?>
			...
			<div class="grid">
				<div class="row">
					...
					function update() {
						$.ajax({
							type: "GET",
							url: 'nodes_netstatjq.php?orderby=<?php echo $orderby; ?>&node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>&filter=<?php echo urlencode($filter); ?>&page=<?php echo $pgkey; ?>&phptimeout=120',
							data: '',
							dataType: 'json',
							cache: false,
							timeout: 120000,
							...
							$('#lastupdate').html('<?php if ($_SESSION['usersett']['nodesnetworkstatsf'] == '') { ?><a href="/xml.php?export=netstat&source=<?php echo $node; ?>" style="font-size:12px;" title="Export Source XML"><div class="icon-file-xml"></div></a><?php } ?>Last&nbsp;Update:&nbsp;' + data.lastupdate);
