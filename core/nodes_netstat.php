...
<?php if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usertype'] == 'Users' || $_SESSION['usersett']['nodesnetworkstats'] > 0) {  } else { header('location: /'); exit; } ?>
...
if (isset($_GET['csv_nodes_netstat'])) {
	$_SESSION['csv_nodes_netstat'] = 'csv_nodes_netstat';
} else {
	$_SESSION['csv_nodes_netstat'] = '';
}

$netstatrrsetting = 5000;

$nodepath = $euryscoinstallpath . '\\nodes\\' . $node . '\\';

?>

<?php if (!isset($_GET['csv_nodes_netstat'])) { $_SESSION['nodes_netstat'] = '<a href="' . htmlspecialchars((string)$_SERVER['REQUEST_URI'], ENT_QUOTES, 'UTF-8') . '" title="Network Stats"><div class="icon-tab"></div>' . $node . '</a>'; } ?>
...
<div class="page secondary">
	<div class="page-header">
		<div class="page-header-content">
            ...
            <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>" class="eurysco-netstat-button big page-back"></a>
		</div>
	</div>
</div>
			...
			<div class="grid">
				<div class="row">
					...
					function update() {
						$.ajax({
							type: "GET",
							url: 'nodes_netstatjq.php?<?php echo substr(md5('$_GET' . $sessiontoken), 5, 15) . '=' . substr(md5('$_GET' . $sessiontoken), 15, 25); ?>&orderby=<?php echo $orderby; ?>&node=<?php echo $node; ?>&domain=<?php echo $domain; ?>&computerip=<?php echo $computerip; ?>&executorport=<?php echo $executorport; ?>&filter=<?php echo urlencode($filter); ?>&page=<?php echo $pgkey; ?>&phptimeout=120',
							data: '',
							dataType: 'json',
							cache: false,
							timeout: 120000,
							...
							$('#lastupdate').html('<?php if ($_SESSION['usersett']['nodesnetworkstatsf'] == '') { ?><a href="/xml.php?<?php echo substr(md5('$_GET' . $sessiontoken), 5, 15) . '=' . substr(md5('$_GET' . $sessiontoken), 15, 25); ?>&export=netstat&source=<?php echo $node; ?>" style="font-size:12px;" title="Export Source XML"><div class="icon-file-xml"></div></a><?php } ?>Last&nbsp;Update:&nbsp;' + data.lastupdate);
