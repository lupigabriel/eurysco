...
if (!strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'] . '/nodes_nagios.php')) { exit; }
...
if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usertype'] == 'Users' || $_SESSION['usersett']['nodesnagiosstatus'] > 0) {  } else { exit; }
...
if (!isset($_SESSION['csv_nodes_nagios'])) {
...
$nagiostable = '<table width="100%" border="0" cellspacing="0" cellpadding="0" class="striped"><tr><td width="1%" align="center"><a href="?orderby=NagiosCommandExit&node=' . $node . '&domain=' . $domain . '&computerip=' . $computerip . '&executorport=' . $executorport . '&filter=' . urlencode($filter) . '" style="font-size:12px; font-weight:bold;' . $obyNagiosCommandExit . '" title="Descending Order by Status">Status</a></td><td width="5%" align="center"><a href="?orderby=NagiosCommandName&node=' . $node . '&domain=' . $domain . '&computerip=' . $computerip . '&executorport=' . $executorport . '&filter=' . urlencode($filter) . '" style="font-size:12px; font-weight:bold; white-space:nowrap; table-layout:fixed; overflow:hidden;' . $obyNagiosCommandName . '" title="Ascending Order by Command Name">Name</a></td><td><a href="?orderby=NagiosCommandMsgs&node=' . $node . '&domain=' . $domain . '&computerip=' . $computerip . '&executorport=' . $executorport . '&filter=' . urlencode($filter) . '" style="font-size:12px; font-weight:bold;' . $obyNagiosCommandMsgs . '" title="Ascending Order by Output Message">Message</a></td></tr>';
...
$lastupdate = 'N/A';
if (!is_null($db->querySingle('SELECT node FROM xmlNagios WHERE node = "' . $node . '"')) || file_exists($nodepath . 'nagios.xml.gz')) {
	if (file_exists($nodepath . 'nagios.xml.gz')) { $lastupdate = date('d/m/Y H:i:s', filemtime($euryscoinstallpath . '\\nodes\\' . $node . '\\nagios.xml.gz')); }
	$xml = simplexml_load_string($db->querySingle('SELECT xml FROM xmlNagios WHERE node = "' . $node . '"'));
	if (!is_object($xml)) {
		$fp = gzopen($nodepath . 'nagios.xml.gz', 'rb');
		$bl = '';
		while (!feof($fp)) {
			$gz = gzread($fp, 2048);
			$bl = $bl . $gz;
	...
	foreach ($xml->children() as $prop=>$n) {
		...
		$nagioscommandname = strtolower(urldecode($xml->$prop->NagiosCommandName));
		$nagioscommandstrn = urldecode($xml->$prop->NagiosCommandStrn);
		$nagioscommandmsgs = urldecode($xml->$prop->NagiosCommandMsgs);
		$nagioscommandexit = $xml->$prop->NagiosCommandExit;
		...
		$prefilter = $_SESSION['usersett']['nodesnagiosstatusf'];
		$checkprefilter = 1;
		...
		if ($checkfilter == 0) {
			...
			if ($orderby == 'NagiosCommandExit') { $nagiosarray[$nagioscounter][0] = $nagiosexitorder; } else { $nagiosarray[$nagioscounter][0] = strtolower($xml->$prop->$orderby); }
...
if ($_SESSION['csv_nodes_nagios'] == 'csv_nodes_nagios') {
	$tmpcsvexport = array();
...
foreach ($nagiosarray as $nagiosrow) {
	array_push($nagiospagearray, '<tr class="rowselectsrv" title="' . htmlentities($nagiosrow[7]) . '"><td style="font-size:12px;" align="center">' . $nagiosrow[5] . '</td><td style="font-size:12px;" align="center" title="' . htmlentities($nagiosrow[2]) . '">' . htmlentities($nagiosrow[1]) . '</td><td style="font-size:12px;">' . htmlentities($nagiosrow[3]) . '</td></tr>');
	if ($_SESSION['csv_nodes_nagios'] == 'csv_nodes_nagios') { array_push($tmpcsvexport, '"' . trim($nagiosrow[1]) . '";' . '"' . $nagiosrow[2] . '";' . '"' . $nagiosrow[3] . '";' . '"' . $nagiosrow[4] . '";' . "\n"); }
}

if ($_SESSION['csv_nodes_nagios'] == '') { $csvexport = '<div style="font-size:12px; white-space:nowrap; table-layout:fixed; overflow:hidden;"><a href="/nodes_nagios.php?csv_nodes_nagios&orderby=' . $orderby . '&node=' . $node . '&domain=' . $domain . '&computerip=' . $computerip . '&executorport=' . $executorport . '&filter=' . urlencode($filter) . '" style="font-size:12px; color:#777;" title="Create CSV"><div class="icon-download-2"></div></a>&nbsp;Total Elements:</div>'; }
if ($_SESSION['csv_nodes_nagios'] == 'csv_nodes_nagios') {
	$_SESSION['csv_nodes_nagios'] = $tmpcsvexport;
}
if ($_SESSION['csv_nodes_nagios'] != '') { $csvexport = '<div style="font-size:12px; white-space:nowrap; table-layout:fixed; overflow:hidden;"><a href="/csv.php?export=csv_nodes_nagios&source=' . $node . '" style="font-size:12px;" title="Export CSV"><div class="icon-download-2"></div></a>&nbsp;Total Elements:</div>'; }
...
if ($nagioscounter == 0) { $nagiostable = $nagiostable . '<tr class="rowselectsrv"><td style="font-size:12px;" align="center" colspan="3">No Results...</td></tr>'; }
...
if (count($nagiospages) > 1) {
	...
	for($i = 1; $i < count($nagiospages) + 1; $i++) {
		...
		if ($i > $pgkey - 5 && $i < $pgkey + 7) {
			$nagiospaging = $nagiospaging . '<a href="?page=' . $i . '&orderby=' . $orderby . '&node=' . $node . '&domain=' . $domain . '&computerip=' . $computerip . '&executorport=' . $executorport . '&filter=' . urlencode($filter) . '"><span class="label" style="' . $selected . '">' . $i . '</span></a>&nbsp;&nbsp;';
		}
	}
	if (count($nagiospages) > $pgkey + 6) {
		$nagiospaging = $nagiospaging . '<div class="icon-flickr" style="font-size:12px; color:#AAAAAA;"></div>&nbsp;&nbsp;<a href="?page=' . count($nagiospages) . '&orderby=' . $orderby . '&node=' . $node . '&domain=' . $domain . '&computerip=' . $computerip . '&executorport=' . $executorport . '&filter=' . urlencode($filter) . '"><span class="label" style="font-size:12px; font-weight:normal; text-align:center; background-color:#CCCCCC; color:#000000; margin:1px; width:30px">' . count($nagiospages) . '</span></a>';
	}
