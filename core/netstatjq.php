...
foreach ($netstatrarray as $netstatrow) {
	...
	if ($_SESSION['usertype'] == 'Administrators' || $_SESSION['usertype'] == 'Operators' || $_SESSION['usertype'] == 'Users' || $_SESSION['usersett']['processcontrol'] > 0) { $NetStatToProcPID = '<a href="/processes.php?filter=IDProcess.' . $netstatrow[5] . '..IDProcess" style="font-size:12px;" title="Filter Processes by PID">' . $netstatrow[5] . '</a>'; $NetStatToProcesses = '<a href="/netstat.php?filter=ProcessName.' . $netstatrow[6] . '..ProcessName" style="font-size:12px;" title="Filter Network Stats by Process">' . $LimitProcess . '</a>'; } else { $NetStatToProcPID = $netstatrow[5]; $NetStatToProcesses = $LimitProcess; }
