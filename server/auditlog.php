...
if (isset($_POST['auditlog'])) {
	$auditlog = base64_decode($_POST['auditlog']);
} else {
	$auditlog = '';
}
...
if (isset($auditlog)) {
	if ($auditlog != '') {
		...
		$dbaudit->close();
		exec('eventcreate.exe /l "Application" /t INFORMATION /so "eurysco Audit : ' . $auditsec[1] . ' : ' . $auditsec[2] . '" /id 1 /d "' . str_replace('"', '\'', $auditlog) . '"', $errorarray, $errorlevel);
	}
}
