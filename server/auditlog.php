...
if (isset($_POST['auditlog'])) {
	if ($_POST['auditlog'] != '') {
		$fp = fopen(str_replace('\\server', '\\core', $_SERVER['DOCUMENT_ROOT']) . '\\audit\\audit-' . date('Ym') . '_' . date('M-Y') . '.log', 'a');
		fwrite($fp, $_POST['auditlog'] . "\r\n");
		fclose($fp);
		$auditsec = explode('     ', $_POST['auditlog']);
		$dbaudit = new SQLite3(str_replace('\\server', '\\sqlite', $_SERVER['DOCUMENT_ROOT']) . '\\euryscoAudit');
